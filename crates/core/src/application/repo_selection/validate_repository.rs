...
pub async fn validate_repository(svc: &RepoSelection, path: &str) -> AppResult<Repository> {
    ...
    if path_trim.is_empty() {
        return Err(AppError::InvalidPath { path: path.to_string() });
    }

    if !svc.git.is_git_repo(path_trim).await.unwrap_or(false) {
        return Err(AppError::NotAGitRepo { path: path_trim.to_string() });
    }
    ...
    let remote = svc
        ...
        .await?
        .ok_or_else(|| AppError::NoGithubRemote { path: path_trim.to_string() })?;

    let parsed = RemoteUrl::parse_github(&remote)
        .ok_or_else(|| AppError::NoGithubRemote { path: path_trim.to_string() })?;
