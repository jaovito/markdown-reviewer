...
- Shiki (`markdown-preview/lib/rehypeShiki.ts`, `lib/highlighter.ts`) highlights fenced code blocks *after* `rehypeSanitize` runs in the pipeline (`markdown-preview/lib/pipeline.ts`), not before. Shiki emits inline `style` on nearly every `<span>`; admitting `style` into the document allowlist to accommodate that would weaken the schema for every element, permanently, to serve one generator. Running afterwards is safe because Shiki's input is already-sanitized text and Shiki escapes what it emits — it's a generator producing markup from text, never a passthrough for author HTML — so it needs no allowlist of its own the way Mermaid's SVG does. It builds one `createHighlighterCoreSync` (JS RegExp engine, a curated static language set, GitHub light/dark themes) at module load so `renderMarkdown` stays synchronous; an unrecognized or absent fence language falls back to plain text inside the same `.shiki` container rather than throwing. Grammar tokenization also has a real cost — measured at ~0.85ms/line against this repo's own source — so fences past `MAX_HIGHLIGHTED_LINES` (600, chosen to cap the blocking `useMemo` in `MarkdownPreview.tsx` at roughly half a second) highlight as `text` regardless of their declared language; this is the fenced-code-block half of CLAUDE.md's "Mermaid and oversized code blocks have fallbacks" rule. `mdast-util-to-hast`'s `code` handler puts `data-source-line` on the `<code>` it builds, not the `<pre>` wrapper around it — `rehypeShiki` reads it from there and re-applies it to the `<pre>` it substitutes in, since the diff gutter and comment anchoring key off that attribute and Shiki replaces the node outright.
- Comment anchoring's `splitCodeBlocks` (`comments/components/InlineThreads.tsx`) has to special-case Shiki's output: Shiki already wraps each source line in its own `<span class="line">` carrying the colored token spans, separated by `"\n"` text-node siblings. `splitCodeBlocks` stamps `data-code-line`/`data-source-line` onto those *existing* spans (absorbing the separator text nodes so `[data-code-line]`'s `display: block` doesn't double-space the block) rather than rebuilding from `textContent` the way it does for the bare `<pre><code>` shape `rehypeShiki`'s catch path can still produce — rebuilding always would silently strip every highlighted block's colors the moment it got a comment. For the same reason, `index.css`'s `.prose-styles .shiki span` rule is scoped to `span[style]`: the comment UI (thread cards, range headers, minimized badges) mounts *inside* the highlighted `<pre>` via direct DOM injection and portals, and a bare `.shiki span` selector would recolor its spans too since CSS descendant selectors don't know they arrived through a portal. Every Shiki token span carries its own inline `style`; the comment UI's spans don't unless they set one explicitly, in which case that inline declaration already outranks the stylesheet rule regardless of whether it matches.
- `src-tauri/tauri.conf.json`'s `app.security.csp` is the second layer behind that sanitizer: even if the allowlist grows a hole, `script-src 'self'` (no `unsafe-inline`, no `unsafe-eval`) blocks script execution outright. `devCsp` is a separate, looser policy (`'unsafe-inline' 'unsafe-eval'` in `script-src`, plus the Vite HMR websocket in `connect-src`) that only applies to `bun run dev` — production never gets it. `img-src` keeps `data:` because the GitHub alert-callout icons are inline `data:image/svg+xml` CSS backgrounds (`shared/styles/index.css`); the document sanitizer separately refuses `data:` in `<img src>`, so this is one layer being deliberately looser than the other, not a conflict. `img-src` also lists both `mdasset:` and `http://mdasset.localhost` because Tauri serves custom schemes over an `http://<scheme>.localhost` origin on Windows. `img-src` deliberately does *not* list generic `http`/`https`: `sanitizeSvg.ts`'s `<image>`/`<use>` `href` allowlist permits external `http`/`https` URLs at the sanitizer layer (Mermaid's `A@{ img: "…" }` node shape needs it), so the CSP is the compensating control that actually stops the browser from fetching them — closing the privacy hole where a remote-hosted diagram image could otherwise beacon a reviewer's IP to whoever hosts it. `<image href>` is unambiguously `img-src`'s request type; `<use href>` on an external URL is not — browser engines have historically disagreed on which directive governs it, some falling back to `default-src`. Since `default-src` is also `'self'` with no generic `http`/`https`, both request types are blocked regardless of which directive a given engine applies, so this is not a gap — but don't read `img-src` alone as the guarantee, and don't "simplify" `default-src` later on the assumption it's redundant with `img-src`. Do not widen either to make a remote diagram image load; that beacon is exactly what they're blocking.
- The `mdasset://` custom URI scheme (`src-tauri/src/asset_protocol.rs`, registered on the `tauri::Builder` in `bootstrap.rs` before `markdown_reviewer_ipc::register`) serves repository images to the WebView so relative Markdown images resolve without exposing a filesystem read primitive. Requests look like `mdasset://localhost/?repo=<urlencoded repo path>&sha=<PR head sha>&path=<urlencoded path relative to repo root>` — parameters travel in the query string, not as path segments, because Windows serves custom schemes over `http://<scheme>.localhost` and handles `mdasset://<host>/<path>` differently than macOS/Linux do. Every request resolves through `application::files::read_repo_asset` (`git show <sha>:<path>` first, the GitHub Contents API as fallback), the same use case `read_markdown_file` mirrors — both resolve inside a git tree object, so `path=../../etc/passwd` resolves to nothing there rather than to a real file. That alone is not sufficient: `git show` also accepts flag-shaped arguments (e.g. `--output=<file>`) anywhere it expects a revision, so an unconstrained `sha` could turn a read into a write, and `sanitizeSchema.ts` allows `mdasset` in `protocols.src` — a crafted `<img src>` in reviewed PR Markdown reaches this handler with zero clicks. `asset_protocol.rs` closes this with two checks before any git/gh call: `is_plausible_sha` rejects any `sha` that isn't a plausible abbreviated-or-full hex object name (`^[0-9a-fA-F]{7,40}$`), and `repo_is_known` rejects any `repo` that isn't an entry in `RecentsStore` — i.e. a repository the user has actually opened through the app's own repo-selection flow, not an arbitrary directory reachable on disk. `infra::git::git_cli`'s `show_file`/`show_file_bytes` additionally pass `--end-of-options` to `git show` as defense in depth, so the argv-injection class is closed even for a caller that skips the `sha` check. `read_repo_asset` also caps the response at `MAX_ASSET_BYTES` (10 MiB, `crates/core/src/application/files/read_repo_asset.rs`) — large enough for any screenshot a doc would reasonably embed, small enough that a stray binary can't pin memory or stall the WebView's image decode; the check is `>`, so an asset at exactly 10 MiB is still served. On any failure — a malformed URI, a missing query parameter, a rejected `sha`/`repo`, an unresolved `sha`/`path`, or an over-cap asset — the handler answers with a blank-body `400`/`404`/`503`; the underlying `AppError` (which can carry a path or an already-redacted process-error string) is only ever written to the local `tracing` log, never into the response the WebView observes. The `200` response also carries `X-Content-Type-Options: nosniff` and `Content-Security-Policy: default-src 'none'` — inert today since nothing can navigate to `mdasset://` directly, but cheap insurance against a future caller that could.
